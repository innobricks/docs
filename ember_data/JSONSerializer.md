...
`JSONSerializer`是序列化器基类，在客户端与服务端进行数据交互时，通过序列化(客户端Record->JSON string)和反序列化(JSON string->客户端Record)，使得数据适配后可以满足前后端的要求。适配器主要完成
...
>为了获得最大性能,Ember-Data建议使用`JSONSerializer`的子类实现`RESTSerializer`

####反序列化（服务端JSON string->客户端Record）

当数据从服务端返回时,序列化器会根据不同的情况，发起不同的`extract`动作,根据不同的情况会有如下数据解析方式
...
Ember Data支持使用使用`links`参数代替相关联模型的`ids`，`links`的value值为对应关联类型的GET 请求路径
...
+ extractArray (store, type, payload) 
    >extractArray 在后端返回值是数组的情况下进行数组序列化为对象的操作
...
>在需要进行全局性序列化定制时，可以
    ...
    ```
###序列化（将客户端的Record对象转换为JSON String）
####序列化钩子方法
